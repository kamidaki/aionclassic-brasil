 ...
 * @author -Nemesiss-, KID
 */
public class C_ROUTE_INFO extends AionClientPacket
{
	/**
	 ...
	 * @param opcode
	 */
	public C_ROUTE_INFO(int opcode, State state, State... restStates)
	{
	...
	protected void runImpl()
	{
		if (BannedMacManager.getInstance().isBanned(macAddress)) {
			...
			LoggerFactory.getLogger(C_ROUTE_INFO.class).info("[MAC_AUDIT] " + macAddress + " (" + this.getConnection().getIP() + ") was kicked due to mac ban");
			LoggerFactory.getLogger(C_ROUTE_INFO.class).info("[Hard_AUDIT] " + HardName + " (" + HardName + ")");
		} else {
